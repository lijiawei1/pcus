package org.zap.framework.common.lang;

import java.math.BigDecimal;
import java.util.Arrays;

/*
 * 系统的浮点数类。统一处理浮点计算，当前算法可处理45位有效数字。
 * 
 ...
 * :张扬 1.新增几个构造子 UFDouble(String,int) 2.修改UFDouble(long,int)错误
 ...
 *  张伟 填加 对科学计数法的支持 UFDouble(String,int) UFDouble(String) 填加一个方法
 *     setValue(Double,int) 修改 UFDouble(double,int) 修改 等于中 的 BUG 修正BUG 在加法后，
 *     2001-09-11 A 进行了调整的原始数据没有调整回来。 B 在加法后进行结果判断时没有判断到小数位
 *  张扬 去掉为0时的负号 2001-11-09.01
 *  张扬 2001-11-23.02 新增属性 trimZero = false;//不压缩小数点后尾部的0
 *  张扬 2001-11-29.01 修改UFDouble(String)、UFDouble(String,int)构造－－四舍五入
 *  张扬 2001-11-30.01 修改UFDouble(String)、UFDouble(String,int)构造－－支持对","的解析
 * 
 * 
 */
public class LDouble extends java.lang.Number implements java.io.Serializable,
        Comparable<LDouble> {
	...
	public static LDouble ONE_DBL = new LDouble(1f);

	public static LDouble ZERO_DBL = new LDouble(0f);
	 ...
	 *
	 */
	public LDouble(double d) throws NumberFormatException {
	 ...
	 * newPower 指数<br>
	 */
	public LDouble(double d, int newPower) throws NumberFormatException {
		setValue(d, newPower);
	}
	 ...
	 * long VALUE
	 */
	public LDouble(long d, int pow) throws NumberFormatException {
		// v1 = d;
		// power = pow;
		this(d + 0.0, pow);
	}
	 ...
	 */
	public LDouble(long[] dv, byte si, int pow) throws NumberFormatException {
	...
	/**
	 *  初始化一个对象
	 * @param str 小数位数默认长度小于9
	 * @throws NumberFormatException
	 */
	public LDouble(String str) throws NumberFormatException {
		String s = "";
	 ...
	 * @param newPower
	 */
	public LDouble(String str, int newPower) {
	 ...
	 */
	public LDouble(java.math.BigDecimal value) {
	 ...
	 * 加上一个数
	 */
	public LDouble add(double d1) {
		return add(new LDouble(d1));
	}

	/**
	 * 加上一个数
	 */
	public LDouble add(LDouble ufd) {
	 ...
	 * 加上一个数
	 */
	public LDouble add(LDouble ufd, int newPower) {
	 ...
	 *            进位方式<br>
	 */
	public LDouble add(LDouble ufd, int newPower, int roundingMode) {
		/** 首先判断POWER的大小 */
		// newPower = newPower > 0 ? -newPower : ((newPower > -9) ? newPower :
		// -9);
		newPower = getValidPower(newPower);

		LDouble fdnew = new LDouble(this);
		//
	...
	private void addUp0(double ufd) {
		addUp0(new LDouble(ufd), power, ROUND_HALF_UP);
	}
	 ...
	 *            进位方式<br>
	 */
	private void addUp0(LDouble ufd, int newPower, int roundingMode) {
		/** 首先判断POWER的大小 */
	...
	public int compareTo(LDouble o) {
		return toDouble().compareTo(((LDouble) o).toDouble());
	}
	...
	public LDouble div(double d1) {
		double d = getDouble() / d1;
		LDouble ufd = new LDouble(d, DEFAULT_POWER);
		ufd.round(ROUND_HALF_UP);
		return ufd;
	}

	public LDouble div(LDouble ufd) {
		double d = getDouble() / ufd.getDouble();
		LDouble ufdNew = new LDouble(d, DEFAULT_POWER);
		ufdNew.round(ROUND_HALF_UP);
		return ufdNew;
	}

	public LDouble div(LDouble ufd, int newPower) {
		// newPower = newPower > 0 ? -newPower : ((newPower > -9) ? newPower :
		// -9);
		newPower = getValidPower(newPower);

		double d = getDouble() / ufd.getDouble();
		LDouble ufdNew = new LDouble(d, newPower);
		ufdNew.round(ROUND_HALF_UP);
		return ufdNew;
	}
	 ...
	 */
	public LDouble div(LDouble ufd, int newPower, int roundingMode) {
		// newPower = newPower > 0 ? -newPower : ((newPower > -9) ? newPower :
		// -9);
		newPower = getValidPower(newPower);

		double d = getDouble() / ufd.getDouble();
		LDouble ufdNew = new LDouble(d);
	...
	public LDouble multiply(double d1) {
		/** 首先判断POWER的大小 */
		LDouble ufD1 = new LDouble(d1);
		return multiply(ufD1, DEFAULT_POWER, ROUND_HALF_UP);
	}

	public LDouble multiply(LDouble ufd) {
		return multiply(ufd, DEFAULT_POWER, ROUND_HALF_UP);
	}

	public LDouble multiply(LDouble ufd, int newPower) {
	 ...
	 *            进位方式<br>
	 */
	public LDouble multiply(LDouble ufd, int newPower, int roundingMode) {
		// newPower = newPower > 0 ? -newPower : ((newPower > -9) ? newPower :
		// -9);
		newPower = getValidPower(newPower);
	 ...
	 *            int
	 */
	private LDouble round(double d, int newPower, int roundingMode) {
		// newPower = newPower > 0 ? -newPower : ((newPower > -9) ? newPower :
		// -9);
		newPower = getValidPower(newPower);
	 ...
	 */
	public LDouble setScale(int power, int roundingMode) {
	...
	public LDouble sub(double d1) {
		LDouble ufd = new LDouble(d1);
		return sub(ufd, DEFAULT_POWER, ROUND_HALF_UP);
	}

	public LDouble sub(LDouble ufd) {
		return sub(ufd, DEFAULT_POWER, ROUND_HALF_UP);
	}

	public LDouble sub(LDouble ufd, int newPower) {
	 ...
	 *            进位方式<br>
	 */
	public LDouble sub(LDouble ufd, int newPower, int roundingMode) {
		// newPower = newPower > 0 ? -newPower : ((newPower > -9) ? newPower :
		// -9);
		newPower = getValidPower(newPower);

		LDouble ufdnew = new LDouble(ufd);
	 ...
	 * 
	 */
	public static LDouble sum(double[] dArray) {
	 ...
	 * 
	 */
	public static LDouble sum(double[] dArray, int newPower) {
		// newPower = newPower > 0 ? -newPower : ((newPower > -9) ? newPower :
		// -9);
		newPower = getValidPower(newPower);

		LDouble ufd = new LDouble(0, newPower);
	...
	public static LDouble sum(double[] dArray, int newPower, int roundingMode) {
		// newPower = newPower > 0 ? -newPower : ((newPower > -9) ? newPower :
		// -9);
		newPower = getValidPower(newPower);
		...
		LDouble ufd = new LDouble(0, newPower);
		for (int i = 0; i < dArray.length; i++) {
			LDouble ufdNew = new LDouble(dArray[i], newPower);
	 ...
	 */
	public LDouble mod(LDouble ufd) {
	 ...
	 *            nc.vo.pub.lang.UFDouble
	 */
	public LDouble mod(LDouble ufd, int newPower) {
	 ...
	 *            nc.vo.pub.lang.UFDouble
	 */
	public LDouble mod(LDouble ufd, int newPower, int roundingMode) {
		LDouble ufdDiv = div(ufd, 0, ROUND_DOWN);
		LDouble ufdnew = sub(ufdDiv.multiply(ufd));
		if (ufd.si != si)
	...
	public boolean equals(Object o) {
		if (o instanceof LDouble) {
			LDouble ud = (LDouble) o;
