...
var manager = {
    ...
    confirmChanged: function() {
        if (this.isChanged()) {
            return confirm("订单[" + data_order.bill_no + "]的" + this.getChangeName() + "数据已被修改，确认关闭？");
        }
        return true;
    },
...
var fields = (function(busi_type) {
    ...
    var mainFormFields = [
        ...
        {display: '揽货公司', name: 'supplier_name', newline: false, cssClass: "field", type: "select", comboboxName: "supplier_name_c",
            options: {data: data_suppliers, isTextBoxMode: true, readonly: true}},
        ...
        {display: '装货报价区域', name: 'load_region', width: defaultFieldWidth.w1, newline: false, cssClass: "field", type: "text", options: {}},
        {display: '详细地址', name: 'load_address', width: defaultFieldWidth.w3, newline: true, cssClass: "field", type: "text",
        ...
        {display: '提柜详细地址', name: 'gate_out_yard', width: defaultFieldWidth.w2, newline: false, cssClass: "field", type: "text"},
        ...
        {display: '还柜详细地址', name: 'gate_in_yard', width: defaultFieldWidth.w2, newline: false, cssClass: "field", type: "text"},
    ...
    //遍历模板
    $.each(MAIN_FORM_TEMPLATE['TYPE_ORDER_'+busi_type.substring(11, busi_type.length)], function (i, v) {
        ...
        $.each(v, function (name, value) {
            ...
            if (fieldOption) {
                ...
                if ('booking_no' === name) {
                    if (busi_type === TMS_BUSI_TYPE.TYPE_FRONT_CNTR_IMPORT) {
                        //进口业务
                        fieldOption.display = '提单号';
                    } else if (busi_type === TMS_BUSI_TYPE.TYPE_FRONT_CNTR_EXPORT) {
                        //出口业务
...
$(window).on("beforeunload",function () {
   if (manager.isChanged()){
       return "订单[" + data_order.bill_no + "]的" + this.getChangeName() + "数据已被修改，确认关闭？";
   }
});
