...
(function ($)
{
    ...
    $.ligerDefaults.Drag = {
        ...
        onStopDrag: false,
        handler: null,
        position: false, //2016-12-09 初始位置获取方式
        posFromCss : false, //2016-12-15 初始位置获取方式，若此项和position同时为true，则此项优先
        bindElem: null, //2017-03-06 指定绑定的元素（选择器），定以后使用on的方式绑定拖拽事件
    ...
    l.controls.Drag.ligerExtend(l.core.UIComponent, {
        ...
        _render: function ()
        {
            ...
            g.mouseDowned = false;
            /**
             * 2017-03-06 新增bindElem属性，指定使用冒泡的方式监听事件元素 
             **/
            if(p.bindElem){
                g.handler.on("mousedown.drag", p.bindElem, mousedown_drag);
                g.handler.on("mousemove.drag", mousemove_drag);
                g.handler.on("mouseup.drag", mouseup_drag);
            }
            else{
                g.handler.on({
                    'mousedown.drag': mousedown_drag,
            ...
            function mousedown_drag(e) {
                if (p.disabled) return;
                if (e.button == 2) return;
                g.mouseDowned = true;
                // 由于没有触发 mouseup_drag 所以就不给予绑定了
                // $(document).on("selectstart.drag", function () { return false; });
                setTimeout(function ()
                {
        ...
        _start: function (e)
        {
            ...
            if (p.disabled) return;
            /**
             * 2016-12-09 新增position选项
             * 2016-12-15 新增posFromCss选项
             * */
            g.current = {
                target: g.target,
            ...
            if(p.posFromCss){
                var cssLeft = g.target.css("left"),
                    cssTop = g.target.css("top");
                g.current.left = getCssPos(cssLeft);
                g.current.top = getCssPos(cssTop);
            }
            else if(p.position){
                g.current.left = g.target.position().left;
                g.current.top = g.target.position().top;
            }
            else{
                g.current.left = g.target.offset().left;
                g.current.top = g.target.offset().top;
            }
            function getCssPos(c){
                if(c === "auto"){
                    return 0;
                }
                else{
