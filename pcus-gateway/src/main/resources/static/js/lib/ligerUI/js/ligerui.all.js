    ...
    $.ligerui.controls.Accordion.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _render: function ()
        {
            ...
            $("> div", g.accordion).each(function (i, box)
            {
                ...
                if (i == selectedIndex)
                    /**
                     * 2016-09-07 给激活的选项添加一个active类
                     * */
                    $(".l-accordion-toggle", header).addClass("l-accordion-toggle-open").parent().addClass("active");
                if ($(box).attr("title"))
                {
            ...
            $(">.l-accordion-header", g.accordion).click(function ()
            {
                ...
                if (togglebtn.hasClass("l-accordion-toggle-close"))
                {
                    ...
                    //添加active类
                    $(this).addClass("active").siblings(".l-accordion-header").removeClass("active")
                        .find(".l-accordion-toggle").removeClass("l-accordion-toggle-open").addClass("l-accordion-toggle-close");
                }
                else
                {
                    ...
                    //移除active类
                    $(this).removeClass("active").next(".l-accordion-content").hide(p.speed);
                }
                $(".l-accordion-header", g.accordion).removeClass("l-accordion-header-downfirst");
                $(".l-accordion-content:visible", g.accordion).next(".l-accordion-header:first").addClass("l-accordion-header-downfirst");
            });
...
//            typeof h == "string"	false
            var ele = "#" + g.element.id + " .togglebtn"
            if (typeof g.element.id == "object") {
            	ele = "#" + g.element.id.name + " .togglebtn"
            }
            //分组 收缩/展开
        ...
        _stop: function (e)
        {
            ...
            else if (g.yresize && g.yresize.diff != undefined)
            {
                ...
                else if (g.dragtype == 'centerbottomresize')
                {
                    ...
                    //调整center.content的高度 by shin
                    var centerHeight = g.center.height() + g.yresize.diff;
                    //alert(centerHeight);
                    g.center.height(centerHeight);
                    if (g.center && g.center.content) g.center.content.height(centerHeight);
                }
            }
            g.trigger('endResize', [{
