...
(function ($)
{
    ...
    $.ligerDefaults.Tree = {
        ...
        data: null,
        checkbox: true,
        checkWhileSelect: false, //2016-12-01 当选择的时候是否响应多选框点击事件
        ...
        single: false,               //是否单选
        singleOpen:false,         //是否只展开一项 2016-09-07
    ...
    $.ligerui.controls.Tree.ligerExtend($.ligerui.core.UIComponent, {
        ...
        update: function (domnode, newnodedata)
        {
            ...
            for (var attr in newnodedata)
            {
                ...
                {
                    /**
                     * 2017-01-04 更新节点时也要调用render函数
                     * */
                    if (p.render)
                    {
                        $("> .l-body > span", domnode).html(p.render(newnodedata, newnodedata[attr]));
                    } else
                    {
        ...
        _getTreeHTMLByData: function (data, outlineLevel, isLast, isExpand)
        {
            ...
            for (var i = 0; i < data.length; i++)
            {
                ...
                /**
                 * 2016-12-01 修复bug，判断此类的时候是判断li元素
                 * */
        ...
        _setTreeEven: function ()
        {
            ...
            g.tree.click(function (e)
            {
                ...
                //chekcbox even
                /**
                 * 2016-12-01 新增checkWhileSelect属性，当选择的时候是否响应多选框点击事件
                 * */
                if ($(obj).hasClass("l-checkbox") || (p.checkWhileSelect && !clickOnTreeItemBtn))
                {
                    var $checkbox = $(obj).hasClass("l-checkbox") ? $(obj) : treeitem.find(".l-checkbox:eq(0)");
                    if (p.autoCheckboxEven)
                    {
                        //状态：未选中
                        if ($checkbox.hasClass("l-checkbox-unchecked"))
                        {
                            $checkbox.removeClass("l-checkbox-unchecked").addClass("l-checkbox-checked");
                            ...
                            //状态：选中
                        else if ($checkbox.hasClass("l-checkbox-checked"))
                        {
                            $checkbox.removeClass("l-checkbox-checked").addClass("l-checkbox-unchecked");
                            $(".l-children .l-checkbox", treeitem)
                            ...
                            //状态：未完全选中
                        else if ($checkbox.hasClass("l-checkbox-incomplete"))
                        {
                            $checkbox.removeClass("l-checkbox-incomplete").addClass("l-checkbox-checked");
                            $(".l-children .l-checkbox", treeitem)
                                    .removeClass("l-checkbox-incomplete l-checkbox-unchecked")
                                    .addClass("l-checkbox-checked");
                            g.trigger('check', [{ data: treenodedata, target: treeitem[0] }, true]);
                    ...
                    //状态：已经张开
                    /**
                     * 2016-12-01 当父元素为选中状态时，不允许通过文本关闭
                     * */
                else if (treeitembtn.hasClass("l-expandable-open") && (!(p.btnClickToToggleOnly || $(">div:first", treeitem).hasClass("l-selected")) || clickOnTreeItemBtn))
                {
                ...
                else if (treeitembtn.hasClass("l-expandable-close") && (!p.btnClickToToggleOnly || clickOnTreeItemBtn))
                {
                    ...
                    $("> div ." + g._getParentNodeClassName(), treeitem)
                            ...
                            .addClass(g._getParentNodeClassName(true));
                    /**
                     * 2016-09-07 扩展：可配置为只展开一项
                     * */
                    if(p.singleOpen){
                        treeitembtn.parents("li[outlinelevel]").siblings("li[outlinelevel]").find(".l-expandable-open").click();
                    }
                }
