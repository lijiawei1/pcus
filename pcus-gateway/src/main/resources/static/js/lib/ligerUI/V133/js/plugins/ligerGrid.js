...
(function ($) {
    ...
    $.ligerDefaults.Grid = {
        ...
        autoStretch: false,                      //2016-10-28 当表格列宽总和小于表格宽度时，是否自动伸展表格列宽
        dataChangedConfirm: null,                 //2016-12-13 当数据改变时，调用的提醒函数，应返回true或false，以确认是否继续
        ...
        inWindowComplete: false,                   //2016-10-28 当inWindow为true时，是否完全以窗口的高度为准，inWindow会计算元素的偏移，
        ...
        onBeforeMoveRow: null,                   // 2017-02-24 行移动前事件
        ...
        localStorageName: null,                // 2017-3-9 本地存储  暂时只保存列
        noHideWidth: true,                    // 2017-3-14 是否去掉隐藏列宽度
        ...
        onColChangeText: null                        // 2017-3-9当列头文本改变时
    };
    ...
    $.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _init: function () {
            ...
            if (p.localStorageName) {
                p.optionsColumns = Object.assign([], p.columns);
                p.pageSize = window.localStorage[p.localStorageName + 'PageSize'] ? window.localStorage[p.localStorageName + 'PageSize'] : p.pageSize;
                g.useColumnLocalStorage();
            }
        },
        _render: function () {
            var g = this, p = this.options;
        ...
        _rendered: function () {
            ...
            // 2018/2/8 添加清除本地储存数据
            // TODO: 重新设计一个删除本地缓存的按钮
            if(p.localStorageName) {
                this._addClearStorageButton()
            }
        },
        ...
        _calculateGridBodyHeight: function (h) {
            ...
            if (typeof h == "string" && h.indexOf('%') > 0) {
                /**
                 * 2016-10-21 要减去偏移
                 * 2016-10-28 新增inWindowComplete属性，统一计算方式
                 */
                if (p.inWindow) {
                    if (p.inWindowComplete) {
                        h = $(window).height() * parseInt(h) * 0.01;
                    }
                    else {
                        h = ($(window).height() - parseInt($('body').css('paddingTop')) - parseInt($('body').css('paddingBottom')) ) * parseInt(h) * 0.01;

                        if (p.inWindow || g.grid.parent()[0].tagName.toLowerCase() == "body")
                            h -= (g.grid.offset().top - $("body").offset().top - parseInt($('body').css('paddingTop')));
                    }
                }
                else {
            ...
            if (p.title) h -= 24;
            /**
             * 2016-10-21 当分页工具条不显示时，不计算高度值
             * */
            if (p.usePager && p.pagerRender || !p.scrollToPage) {
                if (!g.toolbar.is(":hidden")) h -= g.toolbar.outerHeight();
            }
        ...
        loadServerData: function (param, clause, sourceType) {
            ...
            var ajaxOptions = {
                ...
                success: function (data) {
                    ...
                    var setedData = g.trigger('setsuccessdata', [data, g]);
                    data = setedData ? setedData : data;
        ...
        _moveData: function (from, to, isAfter) {
            ...
            // 2017-02-24 新增行移动前事件
            if (g.trigger("beforeMoveRow", [fromRow, toRow]) === false) {
                return false;
            }
        ...
        changePage: function (ctype) {
            ...
            if (g.loading) return true;
            /**
             * 2016-12-13 新增自定义提醒函数
             * */
            var dataChangedConfirm = p.dataChangedConfirm || confirm;
            if (p.dataAction != "local" && g.isDataChanged && !dataChangedConfirm(p.isContinueByDataChanged)) {
                return false;
            }
        ...
        toggleCol: function (columnparm, visible, toggleByPopup) {
            ...
            var column = g.getColumn(columnparm)
            if (!column) {
                return
            }
            ...
            g._setColumnVisible(column, !visible)
            visible ? $(cells).show() : $(cells).hide()
            g.resetColumnsWidth()
            ...
            // (2017年3月6日)
            if (p.localStorageName) {
                var name = column.storageName || column.name
                var newColumn = JSON.parse(localStorage[p.localStorageName]).map(function(item) {
                    if (item.name === name) {
                        item.hide = column._hide
                    }
                    return item
                })
                g.setColumnLocalStorage(newColumn)
            }
            column.hide = column._hide
            // (2017年3月6日)  补充 onToggleCol 事件
            g.trigger('toggleCol', [column, column._hide, visible])
        },
        /**
         * 2016-12-30 新增方法，重新设置列宽
         * */
        resetColumnsWidth: function (ignoreHideCol) {
            var g = this, p = this.options;
            var columns = g.columns,
                col,
                init = Number(g.enabledDetail()) + Number(g.enabledCheckbox()) + Number(g.options.rownumbers);
            ignoreHideCol = ignoreHideCol ||  true;
            for (var i = init, len = columns.length; i < len; i++) {
                col = columns[i];
                if (ignoreHideCol && col._hide) continue;
                col._width = 0;
                col._width = g._getColumnWidth(col);
            }

            if (p.autoStretch) {
                g._autoStretch(ignoreHideCol);
            }

            var gridtablewidth = 0;

            for (i = 0; i < init; i++) {
                col = columns[i];
                gridtablewidth += col._width + 1;
            }

            for (i = init; i < len; i++) {
                col = columns[i];
                if (ignoreHideCol && col._hide) continue;
                g.setColumnWidth(col, col._width);
                gridtablewidth += col._width + 1;
            }

            g.gridtablewidth = gridtablewidth;
        ...
        changeHeaderText: function (columnparm, headerText) {
            ...
            // (2017年3月6日) 添加列头部改变后的事件
            g.trigger('colChangeText', [column, headerText, column.display]);
        },
        //改变列的位置
        changeCol: function (from, to, isAfter) {
            ...
            var fromColumns = fromCol['__pid'] == -1 ? columns : g._columns[fromCol['__pid']].columns;
            var toColumns = toCol['__pid'] == -1 ? columns : g._columns[toCol['__pid']].columns;
            ...
            // 2017年3月9日 修改改变方法
            fromCol.frozen = toCol.frozen;
            columns.splice(fromColIndex, 1);
            columns.splice(toColIndex, 0, fromCol);
            g._setColumns(columns);
            g.reRender();


            p.localStorageName && (function (g, columns) {
                if (!localStorage[p.localStorageName]) {
                    return false;
                }
                // 已存储列表信息
                var oldColumn = JSON.parse(localStorage[p.localStorageName]);
                var oldColumnHash = {};
                oldColumn.forEach(function (item, index) {
                    if (!item.index) {
                        item.index = index;
                    }
                    oldColumnHash[item.name] = item;
                });

                //需要拿出历史宽度
                g.setColumnLocalStorage(columns.map(function (item, index) {
                    //记录的宽度
                    var oldConfig = oldColumnHash[item.storageName || item.name];
                    var oldWidth = (!!oldConfig ? oldConfig.width  : 0);
                    return {
                        name: item.storageName || item.name,
                        width: oldWidth || item.width,
                        index: index,
                        hide: item.hide
                    };
                }));
            })(g, columns);
            // (2017年3月6日)  添加列改变后保存回调的事件
            g.trigger('colChangePlace', [fromColumns, toColumns, fromColIndex, toColIndex, p.columns]);
        },
        ...
        // 2017 - 3 - 13 添加 修改标题的方法
        setTitle: function (text, img) {
            var g = this, p = this.options;
            $(".l-panel-header-text", g.header).html(text);
            if (img) {
                g.header.append("<img src='" + img + "' />").addClass("l-panel-header-hasicon");
            }
        },
        ...
        _initColumns: function () {
            ...
            if (g.enabledCheckbox()) {
                ...
                /**
                 * 2016-08-29 这里的宽度错误地使用了detailColWidth
                 * */
                var col = {ischeckbox: true, issystem: true, width: p.checkboxColWidth, frozen: frozenCheckbox};
                setColumn(col, 1, -1, lastid);
                lastid = col['__id'];
            }
            ...
            /**
             * 2016-10-28 新增属性，autoStretch
             * */
            if (p.autoStretch) {
                g._autoStretch(false);
            }
        },
        /**
         * 2016-12-30 将autoStretch独立出来
         * */
        _autoStretch: function (ignoreHideCol) {
            var g = this, p = this.options;
            ...
            var gridWidth = g.grid.width(),
                ...
                init = Number(g.enabledDetail()) + Number(g.enabledCheckbox()) + Number(g.options.rownumbers);
            for (i = 0; i < len; i++) {
                if (ignoreHideCol && columns[i]._hide) continue;
                colWidth += columns[i]._width;
                clen++;
            }
            if (clen <= 0) return;
            if (colWidth < gridWidth) {
                var lenDiff = Math.floor((gridWidth - colWidth - clen) / (clen - init)),
                    diff = (gridWidth - colWidth - clen) % (clen - init);

                for (i = init; i < len; i++) {
                    if (ignoreHideCol && columns[i]._hide) continue;
                    columns[i]._width += lenDiff;
                }
                columns[--i]._width += diff;
            }
        },
        _getColumnWidth: function (column) {
            ...
            if (typeof (colwidth) == "string" && colwidth.indexOf('%') > 0) {
                ...
                /**
                 * 2016-10-24 （这里没有作修改，仅作记录，当有意将列宽度总和设为100%时，这种计算方式有误差，但是这里要考虑定义列宽时是混合定义）
                 * 一时间也没有比较完美的办法解决，建议直接在具体的页面中修改css
                 * 2016-10-28 改为向下取整
                 * */
                column._width = colwidth = Math.floor(parseInt(colwidth) * 0.01 * (g.grid.width() - lwidth - g.columns.length ));
        ...
        _initBuildGridHeader: function () {
            ...
            for (var level = 1; level <= g._columnMaxLevel; level++) {
                ...
                columns.forEach(function (column, Index) {
                    ...
                    if (column['__leaf']) {
                        // 2017年3月7日 去掉隐藏域宽度
                        var colwidth = (column['_hide'] || column['hide']) && p.noHideWidth ? 0 : column['_width'];
                        if (!column.frozen)
        ...
        _initFootbar: function () {
            ...
            if (p.usePager) {
                ...
                var optStr = "";
                var selectedIndex = -1;
                p.pageSizeOptions.forEach(function (item, i) {
                    if (parseInt(p.pageSize) === item) {
                        selectedIndex = i;
                    }
                    ...
                    $(".l-bar-selectpagesize select", g.toolbar).ligerComboBox(
                        {
                            onBeforeSelect: function () {
                                 ...
                                 * */
                                var dataChangedConfirm = p.dataChangedConfirm || confirm;
                                if (p.url && g.isDataChanged && !dataChangedConfirm(p.isContinueByDataChanged))
                                    return false;
                                return true;
                            },
                            width: 45
        ...
        _showData: function (sourceType) {
            ...
            if (!data || !data.length) {
                ...
                // 2017-3-24 添加新事件
                g.trigger('showDataed', [g.currentData]);
                return;
            } else {
        ...
        _getHtmlFromData: function (data, frozen) {
            ...
            for (var i = 0, l = data.length; i < l; i++) {
                ...
                //自定义css class
                if (p.rowClsRender) {
                    // 2016-11-30 有必要传入所有数据
                    var rowcls = p.rowClsRender(item, rowid, data, i);
        ...
        _applyEditor: function (obj) {
            ...
            if (column.editor.type && p.editors[column.editor.type]) {
                ...
                var editor = p.editors[column.editor.type],
                    jcell = $(rowcell),
                    parentDom = column.frozen ? g.gridview1.find('.l-grid-body') : g.gridview2.find('.l-grid-body'),
                    width = jcell.width(),
                    height = jcell.height(),
                //加上括号解决不能正常判定topBar的高度。不加括号会忽略后面运算出来的值
                    topbarHeight = (p.toolbar ? g.topbar.parent().outerHeight() : 0) + (p.title ? g.header.outerHeight() : 0),
                    jcellLeft = jcell[0].offsetLeft - 1,
                    jcellTop = jcell[0].offsetTop + topbarHeight - 1,
                    pc = jcell.position(),
                ...
                jcellLeft += p.editorLeftDiff || 0;
                jcellTop += p.editorTopDiff || 0;
                height += p.editorHeightDiff || 0;

                // 2017-3-10 滚动条偏移
                var maxLeft = jcellLeft + jcell.width() + 20,
                    maxTop = jcellTop + jcell.height() + 20;
                if (parentDom.scrollLeft() + parentDom.width() < maxLeft) {
                    var scrolleft = maxLeft - parentDom.width();
                    parentDom.scrollLeft(scrolleft);
                } else if (parentDom.scrollLeft() > jcellLeft) {
                    parentDom.scrollLeft(jcellLeft - 10);
                }
                if (parentDom.scrollTop() + parentDom.height() < maxTop) {
                    parentDom.scrollTop(maxLeft - parentDom.height());
                } else if (parentDom.scrollTop() > jcellTop) {
                    parentDom.scrollTop(jcellTop - 10);
                }
                // 2016-10-28 设置编辑单元格的大小，显示边框
                var container = $("<div class='l-grid-editor' style='width: " + width + "px;height:" + height + "px'></div>").appendTo(parentDom);
                container.css({left: jcellLeft, top: jcellTop}).show();
                if (column.textField) editParm.text = g._getValueByName(rowdata, column.textField);
            ...
            try {
                ...
                if (data && data.length) {
                    ...
                    for (var i = 0; i < data.length; i++) {
                        ...
                        //2018-02-26 增加合计的自定义计算方法
                        if (column.totalSummary.calValue) {
                            value = column.totalSummary.calValue(data[i]);
                        }
        ...
        _getTotalSummaryHtml: function (data, classCssName, frozen) {
            ...
            $(g.columns).each(function (columnindex, column) {
                ...
                column._hide && totalsummaryArr.push('style="display: none"');
                ...
                var align = (!!column.totalSummary ? (column.totalSummary.align || column.align): column.align) || 'center';
                totalsummaryArr.push(' style="text-Align:' + align + ';"');
                // if (column.align) {
        ...
        _bulidTotalSummary: function (frozen) {
            ...
            var summaryData = p.dataAction === 'local' ? g.data[p.root] : g.currentData[p.root];
            if (Array.isArray(summaryData) && summaryData.length < 1) { return false; }
            var totalRow = $(g._getTotalSummaryHtml(summaryData, null, frozen));
        ...
        _setEvent: function () {
            ...
            $('select', g.toolbar).change(function () {
                // 2016-12-13 新增自定义提醒函数
                var dataChangedConfirm = p.dataChangedConfirm || confirm;
                if (g.isDataChanged && p.dataAction != "local" && !dataChangedConfirm(p.isContinueByDataChanged))
                    return false;
                p.newPage = 1;
                p.pageSize = this.value;
                if (p.localStorageName) {
                    window.localStorage[p.localStorageName + 'PageSize'] = this.value;
                }
            ...
            if ($.fn.ligerDrag && p.colDraggable) {
                ...
                g.gridheader.add(g.f.gridheader).ligerDrag({
                    ...
                    onStartDrag: function (current, e) {
                        ...
                        var gridOffset = g.gridheader.offset();
                        this.validRange = {
                            top: gridOffset.top,
            ...
            if ($.fn.ligerDrag && p.rowDraggable) {
                ...
                /**
                 * 2017-03-06 指定bindElem，让表格的其他单元格可以复制
                 * */
                var bindElem;
                if (g.options.rownumbers) {
                    bindElem = ".l-grid-row-cell-rownumbers";
                }
                else if (g.enabledDetail()) {
                    bindElem = ".l-grid-row-cell-detail";
                }
                else if (g.enabledCheckbox()) {
                    bindElem = ".l-grid-row-cell-checkbox";
                }
                else {
                    bindElem = ".l-grid-row-cell";
                }

                g.gridbody.add(g.f.gridbody).ligerDrag({
                    revert: true, animate: false,
                    bindElem: bindElem,//2017-03-06
                    proxyX: 0, proxyY: 0,
                    proxy: function (draggable, e) {
                        var src = g._getSrcElementByEvent(e);
                        if (src.row) {
        ...
        _onMouseDown: function (e) {
            var g = this, p = this.options;
            /**
             * 2016-12-09 在这里再写一遍，是为了点击滚动条也会关闭编辑
             * */
            // if (g.editor.editing)
            // {
            //     setTimeout(function () {
        ...
        _onContextmenu: function (e) {
            ...
            if (src.row) {
                ...
                var height = $(g.element).height() * 0.7;
                g.popup.css({left: left, top: g.gridheader.height() + 1, height: height < 120 ? "120" : height});
                g.popup.toggle();
                return false;
            }
        },
        // 2017年3月9日 用于扩展列拖拽
        _onContextmenuDrop: function () {
            var g = this, p = this.options;
            g.menuDroptip = g.popup.find(".l-drag-rowdroptip").length == 0 ? $("<div class='l-drag-rowdroptip' style='display:none'></div>").appendTo(g.popup) : g.popup.find(".l-drag-rowdroptip");
            g.popup.ligerDrag({
                revert: true, animate: false,
                proxyX: 0, proxyY: 0,
                proxy: function (draggable, e) {
                    var content = $(e.target, g.popup).html();
                    var proxy = $("<div class='l-drag-proxy' style='display:none'><div class='l-drop-icon l-drop-no'></div></div>").appendTo('body');
                    proxy.append(content);
                    return proxy;
                },
                onStartDrag: function (current, e) {
                    var gridOffset = g.popup.offset(),
                        $this = $(e.target, g.popup);

                    if (!$this.hasClass('l-column-right')) {
                        if (!$this.parent().hasClass('l-column-right')) {
                            return false;
                        }
                    }
                    ;

                    this.draggingColumn = g.getColumn(parseInt($this.parents('tr').eq(0).find(".l-hidden").attr("columnindex")));
                    this.validRange = {
                        top: gridOffset.top,
                        bottom: gridOffset.top + g.popup.height(),
                        left: gridOffset.left - 10,
                        right: gridOffset.left + g.popup.width() + 10
                    };
                },
                onDrag: function (current, e) {
                    var column = this.draggingColumn;
                    ...
                    if (g.menuDropIn == null) g.menuDropIn = -1;
                    var pageX = e.pageX;
                    var pageY = e.pageY;
                    var validRange = this.validRange;
                    if (pageX < validRange.left || pageX > validRange.right || pageY > validRange.bottom || pageY < validRange.top) {
                    ...
                    for (var i = 0, length = $('tr', g.popup).length; i < length; i++) {
                        var col = $('tr', g.popup).eq(i),
                            colCumn = g.getColumn(parseInt(col.find(".l-hidden").attr("columnindex")));
                        if (colCumn == column) {
                            continue;
                        }
                        ;
                        ...
                        var range = {
                            top: offset.top,
                            bottom: offset.top + 10,
                            left: offset.left - 10,
                            right: offset.left + col.width()
                        };
                        var cellwidth = col.height();
                        range.top += cellwidth;
                        range.bottom += cellwidth;
                        if (pageX > range.left && pageX < range.right && pageY > range.top && pageY < range.bottom) {
                            var lineTop = range.top - g.popup.find('table').offset().top;
                            g.menuDroptip.css({
                                top: lineTop,
                                width: col.width()
                            }).show();
                            g.menuDropIn = g.getColumn(parseInt(col.find(".l-hidden").attr("columnindex"))).__id;
                            this.proxy.find(".l-drop-icon:first").removeClass("l-drop-no l-drop-yes").addClass("l-drop-add");
                            break;
                        } else if (g.colDropIn != -1) {
                            g.menuDropIn = -1;
                ...
                onStopDrag: function (current, e) {
                    if (g.menuDropIn != -1) {
                        var column = this.draggingColumn,
                            col = g.getColumn(g.menuDropIn),
                            visit = false;
                        if (column == col) visit = true;
                        var sameLevel = col['__level'] == column['__level'];
                        var isAfter = !sameLevel ? false : visit ? true : false;
                        if (column.frozen != col.frozen) isAfter = col.frozen ? false : true;
                        var menuDir = isAfter ? "right" : "left";
                        g.changeCol.ligerDefer(g, 0, [column, g.menuDropIn, menuDir == "right"]);
                        g.menuDropIn = -1;
                    }
                    g.menuDroptip.hide();
                    this.set('cursor', 'default');
        ...
        _onDblClick: function (e) {
            ...
            var src = g._getSrcElementByEvent(e);
            /**
             * 2016-11-30 双击树、多选框的时候不触发事件
             * 2016-12-08 双击事件，新增cell参数
             * 2016-12-13 双击明细列时也不触发
             * */
            if (src.row && !src.treelink && !src.checkbox && !src.checkboxcell && !src.detailbtn && !src.detailcell) {
                g.trigger('dblClickRow', [src.data, src.data['__id'], src.row, src.cell]);
            }
        },
        _onClick: function (e) {
            var obj = (e.target || e.srcElement);
            ...
            else if (src.hcelltext) //排序
            {
                 ...
                 * */
                var dataChangedConfirm = p.dataChangedConfirm || confirm;
                if (p.url && p.dataAction != "local" && g.isDataChanged && !dataChangedConfirm(p.isContinueByDataChanged)) return;
                var sort = $(".l-grid-hd-cell-sort:first", hcell);
                /**
                 * 2016-11-28 新增参照字段排序属性值设置
                 * */
                var columnName = src.column.sortname || src.column.name;
        ...
        _onResize: function () {
            ...
            if (p.height && p.height != 'auto') {
                ...
                var h = 0;
                //var parentHeight = null; 2016-10-28
                if (typeof (p.height) == "string" && p.height.indexOf('%') > 0) {
                    ...
                    if (p.inWindow) {
                        if (p.inWindowComplete) {
                            h = windowHeight * parseInt(p.height) * 0.01;
                        }
                        else {
                            h = (windowHeight - parseInt($('body').css('paddingTop')) - parseInt($('body').css('paddingBottom'))) * parseInt(p.height) * 0.01;
                            if (p.inWindow || gridparent[0].tagName.toLowerCase() == "body")
                                /**
                                 * 2016-10-21 消除动画效果的影响
                                 * */
                                h -= (g.grid.offset().top - $("body").offset().top - parseInt($('body').css('paddingTop')));
                        ...
                        h = gridparent.height() * parseInt(p.height) * 0.01;
                    }
                }
                else {
                    h = parseInt(p.height);
        ...
        // 2017-3-9 结束当前的编辑 并进入上一个单元格的编辑状态(如果位于第一个单元格，进入上一行最后一个单元格)
        endEditToPrev: function () {
            ...
            if (editor) {
                ...
                //获取下一行第一个列对象
                function getNextRowCell() {
                    return jprev.parent("tr").prev(".l-grid-row").find("td:last");
                }
            }
            g.endEdit();
            if (jnext && jnext.length) {
        ...
        // 2017-3-9 结束当前的编辑 并进入上一行单元格的编辑状态
        endEditToPrevRow: function () {
            var g = this, p = this.options;
            var editor = g.editor, jnext = null, jprev = null;
            if (editor) {
                var editParm = editor.editParm,
                ...
                var cell = g.getCellObj(editParm.record, editParm.column);
                jprev = $(cell);
                jnext = $(g.getCellObj(g.getRow(rowIndex - 1), editParm.column));
                rowIndex--;
                //获取到下一个可编辑的行
                while (!g._enabledEditByCell(jnext.get(0))) {
                    jprev = jnext;
        ...
        // 2017-3-9 结束当前的编辑 并进入下一行单元格的编辑状态
        endEditToNextRow: function () {
            var g = this, p = this.options;
            var editor = g.editor, jnext = null, jprev = null;
            if (editor) {
                var editParm = editor.editParm,
        ...
        // 2017-3-9 增加的按键操作
        eventsFun: function (e) {
            var g = this, p = this.options;
            if (!g.editor.editing) {
                return;
            }
            var event = (e || window.e) || arguments.callee.caller.arguments[0];
        ...
        // 2017-3-28 本地存储
        useColumnLocalStorage: function () {
            var g = this, p = this.options;
            // 当前需列表信息
            var columnHash = {};
            var column = p.columns.map(function (item, index) {
                var name = item.storageName || item.name;
                columnHash[name] = item;
                return {
                    'name': name,
                    'index': index,
                    'hide': item.hide,
            ...
            if (!localStorage[p.localStorageName]) {
                g.setColumnLocalStorage(column);
                return;
            }
            // 已存储列表信息
            var oldColumn = JSON.parse(localStorage[p.localStorageName]);
            ...
            column.forEach(function (item, index) {
                var name = item.name;
                var oldConfig = oldColumnHash[name];
                if (oldConfig) {
                    item.index = ((oldConfig.index == null || oldConfig.index == undefined) ? item.index : oldConfig.index);
                    item.width = oldConfig.width || item.width;
                    item.hide = oldConfig.hide || item.hide;
                } else {
                    //新增字段使用当前位置
                    item.index = index;
                }
            ...
            //新顺序的配置
            var resultColumn = column.map(function (item, index) {
                //重新编号保存到本地
                item.index = index;
                var originColumn = columnHash[item.name];
                //设置是否可见
                originColumn.width = item.width;
                originColumn.hide = item.hide;
                return originColumn;
            });
            g.setColumnLocalStorage(column);
            p.columns = resultColumn;
        },
        setColumnLocalStorage: function (data) {
            var p = this.options;
            try {
                window.localStorage.setItem(p.localStorageName, JSON.stringify(data));
            } catch (e) {
                console.error(e);
            }
        },
        clearLocalStorage: function () {
            var p = this.options;
            try {
                p.columns = Object.assign([], p.optionsColumns);
                this._setColumns();
                this.loadData();
            } catch (e) {
                console.error(e);
        ...
        _addClearStorageButton: function () {
            var g = this;
            var x = g.toolbar.children().children().last().before('<div class="l-bar-group" title="还原设置"><div class="l-bar-button l-bar-clear-storage"><span></span></div></div>');
            x.prev().on('click', function () {
                if (window.confirm('你确定要还原表格顺序吗？')) {
                    g.clearLocalStorage();
                }
            });
        }
