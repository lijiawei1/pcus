...
(function ($)
{
    ...
    $.ligerui.controls.Layout.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _render: function ()
        {
            ...
            /**
             * 2016-11-24 top/bottom/left/center/centerbottom/right如果有parent-class属性，则向父元素加入相应的类
             * */
            var parentClass;
            //top
            if ($("> div[position=top]", g.layout).length > 0)
            {
                ...
                //2016-11-24
                parentClass = g.top.content.attr("parent-class");
                if(parentClass){
                    g.top.addClass(parentClass);
                }
            ...
            if ($("> div[position=bottom]", g.layout).length > 0)
            {
                ...
                //2016-11-24
                parentClass = g.bottom.content.attr("parent-class");
                if(parentClass){
                    g.bottom.addClass(parentClass);
                }
            ...
            if ($("> div[position=left]", g.layout).length > 0){
                ...
                g.left.content = $("> div[position=left]", g.left);   // 内容

                parentClass = g.left.content.attr("parent-class");
                if (parentClass) {g.left.addClass(parentClass); }
                // 2016-11-24
                // 添加父级样式
                ...
                if (g.left.content.attr("hidetitle")) {
                    ...
                    // 是否添加可隐藏按钮 toggle
                    var headerHtml = '<div class="l-layout-header">';
                    headerHtml += p.allowLeftCollapse ? '<div class="l-layout-header-toggle"></div>' : '';
                    headerHtml += '<div class="l-layout-header-inner"></div></div>';
                    g.left.header = $(headerHtml); // 直接创建 toggle 和 inner
                    g.left.prepend(g.left.header);
                    if ( p.allowLeftCollapse ) g.left.header.toggle = $(".l-layout-header-toggle", g.left.header); // 在 header 中找出toggle
                    //set title
            ...
            if ($("> div[position=center]", g.layout).length > 0)
            {
                ...
                //2016-11-24
                parentClass = g.center.content.attr("parent-class");
                if(parentClass){
                    g.center.addClass(parentClass);
                }
                ...
                if ($("> div[position=centerbottom]", g.layout).length > 0)
                {
                    ...
                    //2016-11-24
                    parentClass = g.centerBottom.content.attr("parent-class");
                    if(parentClass){
                        g.centerBottom.addClass(parentClass);
                    }
            ...
            if ($("> div[position=right]", g.layout).length > 0)
            {
                ...
                //2016-11-24
                parentClass = g.right.content.attr("parent-class");
                if(parentClass){
                    g.right.addClass(parentClass);
                }
            ...
            /**
             * 2016-11-24 若position=left/right里存在collapse-class则加入此类
             * */
            g.leftCollapse = $('<div class="l-layout-collapse-left '+ 
                (g.left && g.left.content.attr("collapse-class") || "") +
                '" style="display: none; "><div class="l-layout-collapse-left-toggle"></div></div>');
            g.rightCollapse = $('<div class="l-layout-collapse-right '+ 
                (g.right && g.right.content.attr("collapse-class") || "") +
                '" style="display: none; "><div class="l-layout-collapse-right-toggle"></div></div>');
        ...
        _bulid: function ()
        {
             ...
             * 2016-11-23 新增事件监听
             * */
            $("[data-action='toggle-left']", g.left).click(function ()
            {
                g.setLeftCollapse(true);
            });
            $("[data-action='toggle-right']", g.right).click(function ()
            {
                g.setRightCollapse(true);
            });
            //set top
        ...
        { 
            ...
            if (typeof (p.height) == "string" && p.height.indexOf('%') > 0)
            {
                 ...
                 * */
                if (p.inWindow || layoutparent[0].tagName.toLowerCase() == "body")
                    h -= (g.layout.offset().top -  $("body").offset().top - parseInt($('body').css('paddingTop')));
            }
            else
            {
                h = parseInt(p.height);
