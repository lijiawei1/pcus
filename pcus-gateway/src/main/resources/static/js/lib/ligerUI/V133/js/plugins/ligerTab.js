...
(function ($)
{
    ...
    $.ligerDefaults.Tab = {
        height: null,
        heightDiff: 0, // 高度补差 
        tabMargin: 2,//2016-09-05扩展，定义每个tab标签的margin
        tabWidthDiff: 0,//2016-09-05扩展，为页签右侧留出的空间
        autoMove: false,//2016-12-19扩展，选中页签时，是否自动移动页签至可视区域
    ...
    $.ligerui.controls.Tab.ligerExtend($.ligerui.core.UIComponent, {
        ...
        {
            var g = this, p = this.options;
            var sumwidth = 0,tabMargin=p.tabMargin;
            $("li", g.tab.links.ul).each(function ()
            {
                //sumwidth += $(this).width() + 2;
                /**
                 * 2016-09-05 使用自定义的margin
                 * */
                sumwidth += $(this).width() + tabMargin;
            });
            //var mainwidth = g.tab.width();
            /**
             * 2016-09-05 mainwidth需减去自定义的误差，为右侧留出空间
             * */
            var mainwidth = g.tab.width() - p.tabWidthDiff;
            if (sumwidth > mainwidth)
            {
                if (!$(".l-tab-links-left", g.tab).length)
                {
                    //2016-09-05
                    g.tab.links.append('<div class="l-tab-links-left"><span></span></div><div class="l-tab-links-right" style="right:'+ p.tabWidthDiff +'px"><span></span></div>');
                    g.setTabButtonEven();
                }
                return true;
            } else
        ...
        selectTabItem: function (tabid)
        {
            ...
            /**
             * 2016-12-19 选中页签时，将页签显示出来
             * */
            var $selectTabItem = $("li[tabid=" + tabid + "]", g.tab.links.ul);
            if(p.autoMove && $selectTabItem.length > 0){
                var $ulElem = g.tab.links.ul,
                    leftBtnWidth = $(".l-tab-links-left", g.tab).outerWidth(),
                    rightBtnWidth = $(".l-tab-links-right", g.tab).outerWidth(),
                    parentWidth = g.tab.width() - p.tabWidthDiff - leftBtnWidth - rightBtnWidth,
                    currentLeft = $ulElem.position().left,
                    selectTabItemLeft = $selectTabItem.position().left,
                    selectTabItemWidth = $selectTabItem.width();

                currentLeft < 0 && (currentLeft = - currentLeft);

                if(currentLeft > selectTabItemLeft){
                    $ulElem.css("left", (leftBtnWidth-selectTabItemLeft) + "px");
                }
                else if(selectTabItemLeft + selectTabItemWidth - currentLeft > parentWidth){
                    $ulElem.css("left", (leftBtnWidth-(selectTabItemLeft + selectTabItemWidth - parentWidth)) + "px");
                }
            }

            $selectTabItem.addClass("l-selected").siblings().removeClass("l-selected");
        ...
        removeTabItem: function (tabid)
        {
            ...
            /**
             * 2016-09-07 自动选择了前一个页签，也应触发页签选择前、后事件
             * */
            var contentItem = $(".l-tab-content-item[tabid=" + tabid + "]", g.tab.content),
                tabItem     =  $("li[tabid=" + tabid + "]", g.tab.links.ul);
            if (tabItem.hasClass("l-selected"))
            {
                var preTabid=contentItem.prev().attr("tabid");
                if (g.trigger('beforeSelectTabItem', [preTabid]) !== false){
                    contentItem.prev().show();
                    tabItem.prev().addClass("l-selected").siblings().removeClass("l-selected");
                    g.trigger('afterSelectTabItem', [preTabid]);
                }
            }
