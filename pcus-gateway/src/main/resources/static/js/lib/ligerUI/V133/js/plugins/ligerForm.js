...
(function ($)
{
    ...
    $.ligerDefaults.Form.editors.textarea =
    {
        create: function (container, editParm, p)
        {
            ...
            /**
             * 2016-11-29 字段定义的readonly和disabled
             * */
            var ep = editParm.field.options || {};
            if (p.readonly || ep.readonly) editor.attr("readonly", true);
            if (p.disabled || ep.disabled) editor.attr("disabled", true);
    ...
    $.ligerui.controls.Form.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _render: function ()
        {
            ...
            /**
             * 2016-10-20 当表单存在name为“id”的时候会引发bug
             * */
            var elemnetId = (g.element.id && (typeof g.element.id === "string")) ? g.element.id : g.id;
            //if (!g.element.id) g.element.id = g.id;
            //分组 收缩/展开
            $("#" + elemnetId + " .togglebtn").on('click', function ()
            {
            ...
            if (fields && fields.length)
            {
                $(fields).each(function (index, field)
                {
                    ...
                    readonly = (field.readonly || (field.editor && field.editor.readonly)) ? true : false,
                        /**
                         * 2016-10-27 如果下拉框是文本输入模式，将验证设到文本框，而不是隐藏框
                         * */
                    txtInputName = (p.prefixID || "") + ((field.options && field.options.isTextBoxMode && field.comboboxName) || field.textField || field.id || field.name);
                    if (field.validate && !readonly)
                    {
                        ...
                        if(field.validate.messages){
                            field.validateMessage = field.validate.messages;
                            delete field.validate.messages;
                        }
        ...
        _initFieldsHtml: function (e)
        {
            ...
            if (fields && fields.length)
            {
                ...
                /**
                 * 2016-11-02 加上了clearfix，这个就没必要了
                 * */
                 ...
                 * 2016-11-02 
                 * */
                $(".l-group", jform).width(jform.width()).append("<div class='togglebtn limit-select'></div>");
            }
        },
        ...
        setData: function (data)
        {
            ...
            if (g.autoEditors && g.autoEditors.length) {
                g.autoEditors.forEach(function (item, index) {
                    ...
                    if (name && data[name] && control && control.setValue) {
                ...
                try {
                    fields.forEach(function (item, index) {
                        var name = item.name,
                            ...
                            editPrev = tabIndex ? 'tab' + tabIndex + '_' : '',
                            editor = g.editors[editPrev + index];
        ...
        initValidate: function ()
        {
            ...
            var validateOptions = $.extend({
                errorPlacement: function (lable, element)
                {
                    ...
                    /**
                     * 2016-10-24 下拉框，隐藏框，扩展判断
                     * */
                    else if (element.attr("type") == "hidden") {
                        element.siblings(".l-text-combobox").addClass("l-text-invalid");
                    }
                    $(element).removeAttr("title").ligerHideTip();
                ...
                success: function (lable)
                {
                     ...
                     */
                        ...
                        element.siblings(".l-text-combobox").removeClass("l-text-invalid");
                    }
                    else
                    {
                        element.parent().removeClass("l-text-invalid");
        ...
        _buliderSpaceContainer: function (field)
        {
            ...
            out.push('">');
            /**
             * 2016-12-07 整个表单不设置验证，不需要加上星号
             * */
            if (p.validate && field.validate && field.validate.required)
            {
