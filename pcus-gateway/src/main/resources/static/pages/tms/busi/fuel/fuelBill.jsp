...
<div class="dialog-container">
    <form id="download" method="POST" target="_blank" action="${path}/tms/bas/fuelBill/excelTmpl">
...
<script>
    var pageError = false;
    try {
        var carNos = ${car_no}; // 车牌号
        var operUnits = ${oper_unit}; // 结算项目
        var driverNames = ${driver_name}; // 结算对象
        var fuelTypes = ${fuel_type}; // 燃油类型
        var supplierIDs =
        ${suppliers} // 所属公司
    } catch (e) {
        pageError = true;
    ...
    $(function () {
        if (pageError) {
            ...
            carNos = carNos.map(function (item) {
                return {
                    id: item,
                    text: item
                }
            });
            driverNames = driverNames.map(function (item) {
                return {
                    id: item,
                    text: item
                }
        ...
        //上下文路径
        var basePath = rootPath + '/tms/bm/fuelBill/';
        ...
        mainGrid = $('#mainGrid').ligerGrid({
            columns: [
                {
                    ...
                    'display': "项目组",
                    'name': "oper_unit",
                    render: LG.render.ref(operUnits, 'oper_unit'),
            ...
            sortName: 'YEAR,MONTH,OPER_UNIT,CAR_NO',
...
//                render: LG.render.ref(operUnits, 'oper_unit')
                ...
                display: '所属公司',
                name: 'supplier_id',
                render: LG.render.ref(supplierIDs, 'supplier_id')
            }],
            url: basePath + 'loadCarGrid',
            checkbox: true,
            width: 580,
            height: 240,
            dataAction: 'server',
            ...
            colDraggable: true,
            delayLoad: true,
            localStorageName: "FuleBillCarGrid" + user.id + param.no,
            selectRowButtonOnly: true,
            sortName: 'CAR_NO',
            sortOrder: 'DESC',
            parms: {
            ...
            items: [
                {
                    ...
                    click: function () {
                        ...
                        xlsUtil.exp($("#download"), mainGrid, '油耗计提明细' + DateUtil.dateToStr('yyyy-MM-dd') + '.xls', {where: weee});
                    }
                }
            ]
        });
...
//                                        item.driver_name = item.default_driver;
        ...
        function refreshconfirm() {
            $("#mainGrid").attr("action", basePath + "update");
    ...
    function jumpDetailEdit(rowdata) {
        var title = '油耗' + rowdata.car_no + '(' + rowdata.year + '/' + rowdata.month + ')';
        top.f_addTab(rowdata.pk_id, title, rootPath + '/tms/bm/fuelBillFee/loadPage/' + rowdata.pk_id + '?module=' + param.module + '&function=' + param.fun);
    }
</script>
</html>
