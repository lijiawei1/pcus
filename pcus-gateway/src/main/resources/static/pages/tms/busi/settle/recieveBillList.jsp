...
<div id="layout">
    ...
    <div class="content-wrapper grid-wrapper">
        ...
        <div class="quick-search">
            ...
            <a class="qs-item" href="javascript:;" onclick="quickQuery(1,this)">新账单(${xinjilu})</a>
...
<script>
    ...
    var where = JSON2.stringify({
        op: 'and',
        rules: [{field: 'DR', value: '0', op: 'equal'}, {field: 'bill_type', value: '1', op: 'equal'}]
    });
    ...
    var gridOption = {
        columns: [
            ...
            {
                display: '下载', name: 'download', align: 'left', minWidth: 50, width: '3%',
                render: function (item, index) {
            ...
            {
                display: '预览', name: 'preview', align: 'left', minWidth: 50, width: '3%'
            },
            {
                display: '状态',
                name: 'state_name',
            ...
            {
                display: '应收账单总额', name: 'bill_amount', align: 'left', minWidth: 50, width: '15%'
            },
            {
        ...
        colDraggable: true,
        localStorageName: 'RecieveBill' + user.id
    };
    ...
    var btnClick = function (item) {
        switch (item.id) {
            case "add":
                //跳转到新页签 编辑
                //进入客户资料信息页签
                top.f_addTab('recieve_bill_do', '应收账单制作', rootPath + '/tms/settle/makebillrecieve/loadPage'
                        + '?module=' + param.module + '&function=应收账单制作&from=' + basePath + 'loadPage');
                break;
            case "audit":
...
<script>
    defaultSearchFilter["and"].push({field: 'bill_type', value: '1', op: 'equal', type: 'string'});
    $(function () {
        //双击事件
        mainGrid.bind('dblClickRow', function (rowdata) {
            //进入客户资料信息页签
            top.f_addTab(rowdata.pk_id, '应收账单[' + rowdata.bill_no + ']', rootPath + '/tms/settle/recieve/loadCard'
                    + '?module=' + param.module + '&function=应收账单&pk_id=' + rowdata.pk_id);
        });
    });
    //初始化编辑按钮
    $mainGrid.on('click', 'a.row-edit-btn', function (e) {
        ...
        //进入客户资料信息页签
        top.f_addTab(selected.pk_id, '应收账单[' + selected.bill_no + ']', rootPath + '/tms/settle/recieve/loadCard'
                + '?module=' + param.module + '&function=应收账单&pk_id=' + selected.pk_id);
...
//        excel(selected.pk_id, '应付账单[' + selected.com_name + selected.bill_year + '年' + selected.bill_month + '月' + ']明细' + DateUtil.dateToStr('yyyy-MM-dd') + '.xls');
        var filename = '应收账单[' + selected.com_name + selected.bill_year + '年' + selected.bill_month + '月' + ']明细.xls';
        var headers = "结算日期,客户,业务类型,费用名称,应收,含税,提柜码头,装卸单位信息,还柜码头,柜号,柜型,订舱号,船公司,船名,航次,预抵时间,封条号,计费方式,计费单位,备注,工作号,急单类型,提柜堆场,还柜堆场,散货数量,揽货公司,操作单位,录单人,录单时间";
        var names = "settle_date,client_name,bs_type_name,fee_name,total,tax,gate_out_dock,load_org,gate_in_dock,cntr_no,cntr_type,booking_no,ship_corp,ship_name,voyage,cntr_work_time,cntr_seal_no,fee_type_name,fee_unit_name,remark,bill_no,urgen_name,gate_out_yard,gate_in_yard,cargo_qty,supplier_name,oper_name,create_psn,create_time";
