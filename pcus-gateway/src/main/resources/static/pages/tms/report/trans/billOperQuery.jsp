...
<div style="display: none">
    <form id="download" method="POST" target="_blank" action="/tms/report/trans/billOperQuery/exportExcel">
...
<script type="text/javascript">
    //上下文路径
    var basePath = rootPath + '/tms/report/trans/billOperQuery/';
    //页面控件
    ...
    var gridOption = {
        columns: [
            ...
            {
                display: '归属单位', name: 'corpname', align: 'left', minWidth: 50, width: '5%'
            },
            {
                display: '录单', name: 'state_10_new', align: 'left', minWidth: 50, width: '5%', totalSummary: {
            ...
            {
                display: '审单', name: 'state_30_audit', align: 'left', minWidth: 50, width: '5%', totalSummary: {
                align: 'center', //汇总单元格内容对齐方式:left/center/right
                type: 'sum', //汇总类型sum,max,min,avg ,count。可以同时多种类型
                render: function (e) {
                    //汇总渲染器，返回html加载到单元格
            ...
            {
                display: '调度', name: 'state_41_submitted', align: 'left', minWidth: 50, width: '5%', totalSummary: {
                align: 'center', //汇总单元格内容对齐方式:left/center/right
                type: 'sum', //汇总类型sum,max,min,avg ,count。可以同时多种类型
                render: function (e) {
                    //汇总渲染器，返回html加载到单元格
            ...
            {
                display: '运输', name: 'state_45_noticed', align: 'left', minWidth: 50, width: '5%', totalSummary: {
                align: 'center', //汇总单元格内容对齐方式:left/center/right
                type: 'sum', //汇总类型sum,max,min,avg ,count。可以同时多种类型
                render: function (e) {
                    //汇总渲染器，返回html加载到单元格
    ...
    var defaultAction = function (item) {
        switch (item.id) {
            case"search":
                ...
                break;
            case"export":
                xlsUtil.exp($('#download'), mainGrid, "单日调度车次.xls", {where: searchForm.getSearchFormData(true, defaultSearchFilter)})
                break;
            case "refresh":
                mainGrid.reload();
                break;
