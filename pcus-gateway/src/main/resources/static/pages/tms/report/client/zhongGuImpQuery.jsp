...
<div style="display: none">
  <form id="download" method="POST" target="_blank" action="/tms/report/client/zhongGuImpQuery/exportExcel">
...
<script type="text/javascript">
  //上下文路径
  var basePath = rootPath + '/tms/report/client/zhongGuImpQuery/';
  //页面控件
  ...
  var gridOption = {
    columns: [
      ...
      {
        display: '货名', name: 'standard', align: 'left', minWidth: 50, width: '8%'
      },
      {
        display: '箱号', name: 'cntr_no', align: 'left', minWidth: 50, width: '10%'
      },
      {
        display: '箱型', name: 'cntr_type', align: 'left', minWidth: 50, width: '15%'
      },
      {
      ...
      {
        display: '压夜费', name: 'yyf', align: 'left', minWidth: 50, width: '8%',totalSummary: {
        align: 'center', //汇总单元格内容对齐方式:left/center/right
        type: 'sum', //汇总类型sum,max,min,avg ,count。可以同时多种类型
        render: function (e) {
          //汇总渲染器，返回html加载到单元格
      ...
      {
        display: '收货人', name: 'con_name', align: 'left', minWidth: 50, width: '15%'
      },
      {
        display: '航次合计', name: 'total', align: 'left', minWidth: 50, width: '15%'
      }
  ...
  var defaultAction = function (item) {
    switch (item.id) {
      case"search":
        ...
        break;
      case"export":
        xlsUtil.exp($('#download'), mainGrid, "上海中谷进口对账单.xls", {where: searchForm.getSearchFormData(false, defaultSearchFilter)})
        break;
      case "refresh":
        mainGrid.reload();
        break;
    ...
    buttons: [
      {
        ...
        onclick: function (item, dialog) {
          ...
          if(y && m){
            $(".l-panel-header-text").html(y+'年'+m+'月份上海中谷进口对帐明细');
          }
          else if(y==''&& m){
            $(".l-panel-header-text").html(m+'月份上海中谷进口对帐明细');
          }
          else if(y && m==''){
            $(".l-panel-header-text").html(y+'年上海中谷进口对帐明细');
          }else{
            $(".l-panel-header-text").html('上海中谷进口对帐明细');
          }
          mainGrid.set('parms', [{
            name: 'where',
