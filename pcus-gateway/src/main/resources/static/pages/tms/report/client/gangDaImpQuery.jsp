...
<div style="display: none">
  <form id="download" method="POST" target="_blank" action="/tms/report/client/gangDaImpQuery/exportExcel">
...
<script type="text/javascript">
  //上下文路径
  var basePath = rootPath + '/tms/report/client/gangDaImpQuery/';
  //页面控件
  ...
  var gridOption = {
    columns: [
      ...
      {
        display: '客户', name: 'client', align: 'left', minWidth: 50, width: '8%'
      },
      {
        display: '柜号', name: 'cntr_no', align: 'left', minWidth: 50, width: '10%'
      },
      {
        display: '尺寸', name: 'cntr_type', align: 'left', minWidth: 50, width: '8%'
      },
      {
        display: '路线', name: 'dock', align: 'left', minWidth: 50, width: '6%'
      },
      {
        display: '吉/重', name: 'e_f', align: 'left', minWidth: 50, width: '6%'
      },
      {
    ...
    onShowDataed : function(){
      mainGrid.gridview2.find('.l-grid-body-inner').append('<div style="text-align: center;margin-left: -27px;"> <p>注 ：请贵公司核对上述运输清单，并签名确认：</p><p>帐号:444000091018170089605    开户行:珠海市交通银行分行营业部</p><p>户名:珠海市集装箱运输有限公司</p></div>')
    }
    /* ,
     sortName: 'd',
     sortOrder: 'ASC'*/
  ...
  var defaultAction = function (item) {
    switch (item.id) {
      case"search":
        ...
        break;
      case"export":
        xlsUtil.exp($('#download'), mainGrid, "港达进口对账单.xls", {where: searchForm.getSearchFormData(false, defaultSearchFilter)})
        break;
      case "refresh":
        mainGrid.reload();
        break;
    ...
    buttons: [
      {
        ...
        onclick: function (item, dialog) {
          ...
          if(y && m){
            $(".l-panel-header-text").html(y+'年'+m+'月份港达进口对账明细');
          }
          else if(y==''&& m){
            $(".l-panel-header-text").html(m+'月份港达进口对账明细');
          }
          else if(y && m==''){
            $(".l-panel-header-text").html(y+'年港达进口对账明细');
          }else{
            $(".l-panel-header-text").html('港达进口对账明细');
          }
          mainGrid.set('parms', [{
            name: 'where',
