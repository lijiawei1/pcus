...
<div style="display: none">
  <form id="download" method="POST" target="_blank" action="/tms/report/client/anTongQuery/exportExcel">
...
<script type="text/javascript">
  //上下文路径
  var basePath = rootPath + '/tms/report/client/anTongQuery/';
  //页面控件
  ...
  var gridOption = {
    columns: [
      ...
      {
        display: '业务时间', name: 'd', align: 'left', minWidth: 50, width: '10%'
      },
      {
        display: '船名航次', name: 'ship_info', align: 'left', minWidth: 50, width: '10%'
      },
      {
      ...
      {
        display: '装货地', name: 'load_address', align: 'left', minWidth: 50, width: '15%'
      },
      {
        display: '卸货地', name: 'unload_address', align: 'left', minWidth: 50, width: '15%'
      },
      {
        display: '超时费', name: 'csf', align: 'left', minWidth: 50, width: '8%',totalSummary: {
  ...
  var defaultActionOption = {
    items: [
      {id: 'search', text: '查询', click: dialogOpen, icon: 'search', status: ['OP_INIT']},
      {id: 'export', text: '导出', click: function () {
          xlsUtil.exp($('#download'), mainGrid, "安通对账单.xls", {where: searchForm.getSearchFormData(false, defaultSearchFilter)});
      }, icon: 'export', status: ['OP_INIT']},
      {id: 'refresh', text: '刷新', click: function () {
          mainGrid.reload();
      }, icon: 'refresh', status: ['OP_INIT']}
    ]
  };
          ...
          buttons: [
              {
                  ...
                  onclick: function (item, dialog) {
                      ...
                      if(y && m){
                          $(".l-panel-header-text").html(y+'年'+m+'月份安通对帐明细');
                      }
                      else if(y==''&& m){
                          $(".l-panel-header-text").html(m+'月份安通对帐明细');
                      }
                      else if(y && m==''){
                          $(".l-panel-header-text").html(y+'年安通对帐明细');
                      }else{
                          $(".l-panel-header-text").html('安通对帐明细');
                      }
                      mainGrid.set('parms', [{
                          name: 'where',
