...
<div style="display: none">
  <form id="download" method="POST" target="_blank" action="/tms/report/client/haiZhiFengBillQuery/exportExcel">
...
<script type="text/javascript">
  //上下文路径
  var basePath = rootPath + '/tms/report/client/haiZhiFengBillQuery/';
  //页面控件
  ...
  var gridOption = {
    columns: [
      ...
      {
        display: '收货单位名称', name: 'client_name', align: 'left', minWidth: 50, width: '10%'
      },
      {
  ...
  var defaultAction = function (item) {
    switch (item.id) {
      case"search":
        ...
        break;
      case"export":
        xlsUtil.exp($('#download'), mainGrid, "海之峰对账单.xls", {where: searchForm.getSearchFormData(false, defaultSearchFilter)})
        break;
      case "refresh":
        mainGrid.reload();
        break;
    ...
    buttons: [
      {
        ...
        onclick: function (item, dialog) {
          ...
          if(y && m){
            $(".l-panel-header-text").html(y+'年'+m+'月份海之峰对帐明细');
          }
          else if(y==''&& m){
            $(".l-panel-header-text").html(m+'月份海之峰对帐明细');
          }
          else if(y && m==''){
            $(".l-panel-header-text").html(y+'年海之峰对帐明细');
          }else{
            $(".l-panel-header-text").html('海之峰对帐明细');
          }
          mainGrid.set('parms', [{
            name: 'where',
