...
<div style="display: none">
    <form id="download" method="POST" target="_blank" action="/tms/report/client/GLCarrierPayBill/exportExcel">
...
<script type="text/javascript">
    //上下文路径
    var basePath = rootPath + '/tms/report/client/GLCarrierPayBill/';
    //页面控件
    ...
    var gridOption = {
        columns: [
            ...
            {
                display: '洗柜费', name: 'xgf', align: 'left', minWidth: 50, width: '6%',
                totalSummary: {
                    align: 'center', type: 'sum',
                    render: function (info, column) {
                        gridColumnRender.push({index: column.columnindex, isShow: !(info.sum === 0)});
    ...
    var defaultAction = function (item) {
        switch (item.id) {
            case"search":
                ...
                break;
            case"export":
                xlsUtil.exp($('#download'), mainGrid, "通用外协集装箱对账单.xls", {where: searchForm.getSearchFormData(false, defaultSearchFilter)})
                break;
            case "refresh":
                mainGrid.reload();
                break;
