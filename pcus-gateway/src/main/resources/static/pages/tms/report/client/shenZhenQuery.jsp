...
<div style="display: none">
  <form id="download" method="POST" target="_blank" action="/tms/report/client/shenZhenQuery/exportExcel">
...
<script type="text/javascript">
  //上下文路径
  var basePath = rootPath + '/tms/report/client/shenZhenQuery/';
  //页面控件
  ...
  title:'深圳珠港对账明细',
    ...
    mainGrid.gridview2.find('.l-grid-body-inner').append('<div style="text-align: center;margin-left: -27px;"> <p>注 ：请贵公司核对上述运输清单，并签名确认。</p><p>帐号:444000091018170089605    开户行:珠海市交通银行分行营业部</p></div>')
  }
    /* ,
     sortName: 'd',
     sortOrder: 'ASC'*/
  ...
  var defaultAction = function (item) {
    switch (item.id) {
      case"search":
        ...
        break;
      case"export":
        xlsUtil.exp($('#download'), mainGrid, "深圳珠港对账单.xls", {where: searchForm.getSearchFormData(false, defaultSearchFilter)})
        break;
      case "refresh":
        mainGrid.reload();
        break;
    ...
    buttons: [
      {
        ...
        onclick: function (item, dialog) {
          ...
          if(y && m){
            $(".l-panel-header-text").html(y+'年'+m+'月份深圳珠港对账明细');
          }
          else if(y==''&& m){
            $(".l-panel-header-text").html(m+'月份深圳珠港对账明细');
          }
          else if(y && m==''){
            $(".l-panel-header-text").html(y+'年深圳珠港对账明细');
          }else{
            $(".l-panel-header-text").html('深圳珠港对账明细');
          }
          mainGrid.set('parms', [{
            name: 'where',
