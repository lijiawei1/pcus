...
<div style="display: none">
  <form id="download" method="POST" target="_blank" action="/tms/report/client/fanYaQuery/exportExcel">
...
<script type="text/javascript">
  //上下文路径
  var basePath = rootPath + '/tms/report/client/fanYaQuery/';
  //页面控件
  ...
  var gridOption = {
    columns: [
      ...
      {
        display: '到港日期', name: 'arrival_date', align: 'left', minWidth: 50, width: '10%'
      },
      {
        display: '送货日期', name: 'd', align: 'left', minWidth: 50, width: '10%'
      },
      {
      ...
      {
        display: '计费门点', name: 'org', align: 'left', minWidth: 50, width: '8%'
      },
      {
  ...
  var defaultAction = function (item) {
    switch (item.id) {
      case"search":
        ...
        break;
      case"export":
        xlsUtil.exp($('#download'), mainGrid, "上海泛亚(洪湾)对账单.xls", {where: searchForm.getSearchFormData(false, defaultSearchFilter)})
        break;
      case "refresh":
        mainGrid.reload();
        break;
    ...
    buttons: [
      {
        ...
        onclick: function (item, dialog) {
          ...
          if(y && m){
            $(".l-panel-header-text").html(y+'年'+m+'月份上海泛亚(洪湾)对帐明细');
          }
          else if(y==''&& m){
            $(".l-panel-header-text").html(m+'月份上海泛亚(洪湾)对帐明细');
          }
          else if(y && m==''){
            $(".l-panel-header-text").html(y+'年上海泛亚(洪湾)对帐明细');
          }else{
            $(".l-panel-header-text").html('上海泛亚(洪湾)对帐明细');
          }
          mainGrid.set('parms', [{
            name: 'where',
