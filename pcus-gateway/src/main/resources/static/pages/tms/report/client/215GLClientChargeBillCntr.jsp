...
<div style="display: none">
    <form id="download" method="POST" target="_blank" action="/tms/report/client/GLClientChargeBillCntr/exportExcel">
...
<script type="text/javascript">
    //上下文路径
    var basePath = rootPath + '/tms/report/client/GLClientChargeBillCntr/';
    //页面控件
    ...
    var gridOption = {
        columns: [
            ...
            {
                display: '客户', name: 'client_name', align: 'left', minWidth: 50, width: '10%', xlsIgnore: true
            },
            {
                display: '收货单位名称', name: 'unload_org', align: 'left', minWidth: 50, width: '10%'
            },
            {
            ...
            {
                display: '吊装费', name: 'dzf', align: 'left', minWidth: 50, width: '6%',
                totalSummary: {
                    align: 'center', type: 'sum',
                    render: function (info, column) {
                        gridColumnRender.push({index: column.columnindex, isShow: !(info.sum === 0)});
            ...
            {
                display: '合计', name: 'total_amount', align: 'left', minWidth: 50, width: '6%',
                totalSummary: {
                    align: 'center',
                    type: 'sum',
                    render: function (info, column) {
    ...
    var defaultAction = function (item) {
        switch (item.id) {
            case"search":
                ...
                break;
            case"export":
                xlsUtil.exp($('#download'), mainGrid, "通用对账单.xls", {where: searchForm.getSearchFormData(false, defaultSearchFilter)})
                break;
            case "refresh":
                mainGrid.reload();
                break;
        ...
        buttons: [
            {
                ...
                onclick: function (item, dialog) {
                    ...
                    if (y && m) {
                        $(".l-panel-header-text").html(y + '年' + m + '月份' + name + '对帐明细');
                    }
                    else if (y == '' && m) {
                        $(".l-panel-header-text").html(m + '月份' + name + '对帐明细');
                    }
                    else if (y && m == '') {
                        $(".l-panel-header-text").html(y + '年' + name + '对帐明细');
                    } else {
                        $(".l-panel-header-text").html(name + '对帐明细');
                    }
                    mainGrid.set('parms', [{
                        name: 'where',
