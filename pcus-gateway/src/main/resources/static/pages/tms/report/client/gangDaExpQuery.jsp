...
<div style="display: none">
  <form id="download" method="POST" target="_blank" action="/tms/report/client/gangDaExpQuery/exportExcel">
...
<script type="text/javascript">
  //上下文路径
  var basePath = rootPath + '/tms/report/client/gangDaExpQuery/';
  //页面控件
  ...
  var gridOption = {
    columns: [
      ...
      {
        display: '出口日期', name: 'd', align: 'left', minWidth: 50, width: '10%'
      },
      {
        display: '订单号', name: 'booking_no', align: 'left', minWidth: 50, width: '8%'
      },
      {
        display: '柜号', name: 'cntr_no', align: 'left', minWidth: 50, width: '10%'
      },
      {
        display: '尺码', name: 'cntr_type', align: 'left', minWidth: 50, width: '8%'
      },
      {
        display: '数量', name: 'qty', align: 'left', minWidth: 50, width: '8%',totalSummary: {
      ...
      {
        display: '出口口岸', name: 'dock', align: 'left', minWidth: 50, width: '6%'
      },
      {
    ...
    onShowDataed : function(){
      mainGrid.gridview2.find('.l-grid-body-inner').append('<div style="text-align: center;margin-left: -27px;"> <p>结算客户：市集</p><p>统计周期:</p><p>珠海市集装箱运输有限公司</p></div>')
    }
    /* ,
     sortName: 'd',
     sortOrder: 'ASC'*/
  ...
  var defaultAction = function (item) {
    switch (item.id) {
      case"search":
        ...
        break;
      case"export":
        xlsUtil.exp($('#download'), mainGrid, "港达出口对账单.xls", {where: searchForm.getSearchFormData(false, defaultSearchFilter)})
        break;
      case "refresh":
        mainGrid.reload();
        break;
    ...
    buttons: [
      {
        ...
        onclick: function (item, dialog) {
          ...
          if(y && m){
            $(".l-panel-header-text").html(y+'年'+m+'月份港达出口对帐明细');
          }
          else if(y==''&& m){
            $(".l-panel-header-text").html(m+'月份港达出口对帐明细');
          }
          else if(y && m==''){
            $(".l-panel-header-text").html(y+'年港达出口对帐明细');
          }else{
            $(".l-panel-header-text").html('港达出口对帐明细');
          }
          mainGrid.set('parms', [{
            name: 'where',
