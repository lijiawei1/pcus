...
<div style="display: none">
  <form id="download" method="POST" target="_blank" action="/tms/report/client/gangWuLiuWxQuery/exportExcel">
...
<script type="text/javascript">
  //上下文路径
  var basePath = rootPath + '/tms/report/client/gangWuLiuWxQuery/';
  //页面控件
  ...
  var gridOption = {
    columns: [
      ...
      {
        display: '应付运费', name: 'total_amount', align: 'left', minWidth: 50, width: '10%'
      },
      {
        display: '应扣油款、车款', name: '', align: 'left', minWidth: 50, width: '8%'
      },
      {
        display: '实付运费', name: 'total_amount', align: 'left', minWidth: 50, width: '8%'
      },
      {
        display: '实付运费（不含税）', name: 'no_tax_amount', align: 'left', minWidth: 50, width: '10%'
      },
      {
  ...
  var defaultAction = function (item) {
    switch (item.id) {
      case"search":
        ...
        break;
      case"export":
        xlsUtil.exp($('#download'), mainGrid, "珠海港物流外协明细.xls", {where: searchForm.getSearchFormData(false, defaultSearchFilter)})
        break;
      case "refresh":
        mainGrid.reload();
        break;
    ...
    buttons: [
      {
        ...
        onclick: function (item, dialog) {
          ...
          if(y && m){
            $(".l-panel-header-text").html(y+'年'+m+'月份珠海港物流外协明细');
          }
          else if(y==''&& m){
            $(".l-panel-header-text").html(m+'月份珠海港物流外协明细');
          }
          else if(y && m==''){
            $(".l-panel-header-text").html(y+'年珠海港物流外协明细');
          }else{
            $(".l-panel-header-text").html('珠海港物流外协明细');
          }
          mainGrid.set('parms', [{
            name: 'where',
