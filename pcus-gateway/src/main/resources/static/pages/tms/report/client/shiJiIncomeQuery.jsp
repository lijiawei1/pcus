...
<div style="display: none">
  <form id="download" method="POST" target="_blank" action="/tms/report/client/shiJiIncomeQuery/exportExcel">
...
<script type="text/javascript">
  //上下文路径
  var basePath = rootPath + '/tms/report/client/shiJiIncomeQuery/';
  //页面控件
  ...
  var gridOption = {
    columns: [
      ...
      {
        display: 'BP', name: '', align: 'left', minWidth: 50, width: '15%'
      },
      {
  ...
  var defaultAction = function (item) {
    switch (item.id) {
      case"search":
        ...
        break;
      case"export":
        xlsUtil.exp($('#download'), mainGrid, "珠海市集收入明细.xls", {where: searchForm.getSearchFormData(false, defaultSearchFilter)})
        break;
      case "refresh":
        mainGrid.reload();
        break;
    ...
    buttons: [
      {
        ...
        onclick: function (item, dialog) {
          ...
          if(y && m){
            $(".l-panel-header-text").html(y+'年'+m+'月份珠海市集收入明细');
          }
          else if(y==''&& m){
            $(".l-panel-header-text").html(m+'月份珠海市集收入明细');
          }
          else if(y && m==''){
            $(".l-panel-header-text").html(y+'年珠海市集收入明细');
          }else{
            $(".l-panel-header-text").html('珠海市集收入明细');
          }
          mainGrid.set('parms', [{
            name: 'where',
