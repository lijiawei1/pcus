...
<div style="display: none">
  <form id="download" method="POST" target="_blank" action="/tms/report/client/yangYueBillQuery/exportExcel">
...
<script type="text/javascript">
  //上下文路径
  var basePath = rootPath + '/tms/report/client/yangYueBillQuery/';
  //页面控件
  ...
  var defaultAction = function (item) {
    switch (item.id) {
      case"search":
        ...
        break;
      case"export":
        xlsUtil.exp($('#download'), mainGrid, "杨粤对账单.xls", {where: searchForm.getSearchFormData(false, defaultSearchFilter)})
        break;
      case "refresh":
        mainGrid.reload();
        break;
    ...
    buttons: [
      {
        ...
        onclick: function (item, dialog) {
          ...
          if(y && m){
            $(".l-panel-header-text").html(y+'年'+m+'月份杨粤对帐明细');
          }
          else if(y==''&& m){
            $(".l-panel-header-text").html(m+'月份杨粤对帐明细');
          }
          else if(y && m==''){
            $(".l-panel-header-text").html(y+'年杨粤对帐明细');
          }else{
            $(".l-panel-header-text").html('杨粤对帐明细');
          }
          mainGrid.set('parms', [{
            name: 'where',
