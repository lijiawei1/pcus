...
<div style="display: none">
  <form id="download" method="POST" target="_blank" action="/tms/report/client/geLiQuery/exportExcel">
...
<script type="text/javascript">
  //上下文路径
  var basePath = rootPath + '/tms/report/client/geLiQuery/';
  //页面控件
  ...
  var gridOption = {
    columns: [
      ...
      {
        display: '码头', name: 'dock', align: 'left', minWidth: 50, width: '6%'
      },
      {
        display: '卸货地', name: 'unload_org', align: 'left', minWidth: 50, width: '6%'
      },
      {
        display: '提柜日期', name: 'd', align: 'left', minWidth: 50, width: '10%'
      },
      {
  ...
  var defaultAction = function (item) {
    switch (item.id) {
      case"search":
        ...
        break;
      case"export":
        xlsUtil.exp($('#download'), mainGrid, "格力出口对账单.xls", {where: searchForm.getSearchFormData(false, defaultSearchFilter)})
        break;
      case "refresh":
        mainGrid.reload();
        break;
    ...
    buttons: [
      {
        ...
        onclick: function (item, dialog) {
          ...
          if(y && m){
            $(".l-panel-header-text").html(y+'年'+m+'月份格力电器进口重柜对帐明细');
          }
          else if(y==''&& m){
            $(".l-panel-header-text").html(m+'月份格力电器进口重柜对帐明细');
          }
          else if(y && m==''){
            $(".l-panel-header-text").html(y+'年格力电器进口重柜对帐明细');
          }else{
            $(".l-panel-header-text").html('格力电器进口重柜对帐明细');
          }
          mainGrid.set('parms', [{
            name: 'where',
