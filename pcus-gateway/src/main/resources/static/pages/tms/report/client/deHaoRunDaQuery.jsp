...
<div style="display: none">
  <form id="download" method="POST" target="_blank" action="/tms/report/client/deHaoRunDaQuery/exportExcel">
...
<script type="text/javascript">
  //上下文路径
  var basePath = rootPath + '/tms/report/client/deHaoRunDaQuery/';
  //页面控件
  ...
  var gridOption = {
    columns: [
      ...
      {
        display: '垫付费', name: 'dff', align: 'left', minWidth: 50, width: '8%',totalSummary: {
        align: 'center', //汇总单元格内容对齐方式:left/center/right
        type: 'sum', //汇总类型sum,max,min,avg ,count。可以同时多种类型
        render: function (e) {
          //汇总渲染器，返回html加载到单元格
  ...
  var defaultAction = function (item) {
    switch (item.id) {
      case"search":
        ...
        break;
      case"export":
        xlsUtil.exp($('#download'), mainGrid, "德豪润达对账单.xls", {where: searchForm.getSearchFormData(false, defaultSearchFilter)})
        break;
      case "refresh":
        mainGrid.reload();
        break;
    ...
    buttons: [
      {
        ...
        onclick: function (item, dialog) {
          ...
          if(y && m){
            $(".l-panel-header-text").html(y+'年'+m+'月份德豪润达对帐明细');
          }
          else if(y==''&& m){
            $(".l-panel-header-text").html(m+'月份德豪润达对帐明细');
          }
          else if(y && m==''){
            $(".l-panel-header-text").html(y+'年德豪润达对帐明细');
          }else{
            $(".l-panel-header-text").html('德豪润达对帐明细');
          }
          mainGrid.set('parms', [{
            name: 'where',
