...
<div style="display: none">
  <form id="download" method="POST" target="_blank" action="/tms/report/client/qiaoPaiBillQuery/exportExcel">
...
<script type="text/javascript">
  //上下文路径
  var basePath = rootPath + '/tms/report/client/qiaoPaiBillQuery/';
  //页面控件
  ...
  var gridOption = {
    columns: [
      ...
      {
        display: '进口日期', name: 'inport_date', align: 'left', minWidth: 50, width: '10%'
      },
      {
        display: '结算日期', name: 'd', align: 'left', minWidth: 50, width: '10%'
      },
      {
        display: 'PONo', name: 'client_bill_no', align: 'left', minWidth: 50, width: '10%'
      },
      {
        display: '货物名称', name: 'cargo_info', align: 'left', minWidth: 50, width: '8%'
      },
      {
        display: '产品型号', name: 'cargo_code', align: 'left', minWidth: 50, width: '10%'
      },
      {
        display: '柜号', name: 'cntr_no', align: 'left', minWidth: 50, width: '8%'
      },
      {
        display: '进厂日期', name: 'arrival_date', align: 'left', minWidth: 50, width: '8%'
      },
      {
        display: '进厂时间', name: 'arriver_time', align: 'left', minWidth: 50, width: '8%'
      },
      {
        display: '离厂日期', name: 'leave_date', align: 'left', minWidth: 50, width: '8%'
      },
      {
        display: '离厂时间', name: 'leave_time', align: 'left', minWidth: 50, width: '8%'
      },
      {
        display: '压车时间', name: 'yc_time', align: 'left', minWidth: 50, width: '8%'
      },
      {
      ...
      {
        display: '超时费(系统)', name: 'csf_sys', align: 'left', minWidth: 50, width: '6%', totalSummary: {
        align: 'center', //汇总单元格内容对齐方式:left/center/right
        type: 'sum', //汇总类型sum,max,min,avg ,count。可以同时多种类型
        render: function (e) {
          //汇总渲染器，返回html加载到单元格
      ...
      {
        display: '总额', name: 'total', align: 'left', minWidth: 50, width: '6%',totalSummary: {
        align: 'center', //汇总单元格内容对齐方式:left/center/right
        type: 'sum', //汇总类型sum,max,min,avg ,count。可以同时多种类型
        render: function (e) {
          //汇总渲染器，返回html加载到单元格
      ...
      {
        display: '返柜时间', name: 'cntr_ret_time', align: 'left', minWidth: 50, width: '15%'
      },
      {
        display: '车牌', name: 'car_no', align: 'left', minWidth: 50, width: '15%'
      }
  ...
  var defaultAction = function (item) {
    switch (item.id) {
      case"search":
        ...
        break;
      case"export":
        xlsUtil.exp($('#download'), mainGrid, "壳牌对账单.xls", {where: searchForm.getSearchFormData(false, defaultSearchFilter)})
        break;
      case "refresh":
        mainGrid.reload();
        break;
    ...
    buttons: [
      {
        ...
        onclick: function (item, dialog) {
          ...
          if(y && m){
            $(".l-panel-header-text").html(y+'年'+m+'月份壳牌对帐明细');
          }
          else if(y==''&& m){
            $(".l-panel-header-text").html(m+'月份壳牌对帐明细');
          }
          else if(y && m==''){
            $(".l-panel-header-text").html(y+'年壳牌对帐明细');
          }else{
            $(".l-panel-header-text").html('壳牌对帐明细');
          }
          mainGrid.set('parms', [{
            name: 'where',
