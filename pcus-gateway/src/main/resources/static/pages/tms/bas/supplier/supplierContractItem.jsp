...
<script>
    ...
    //上下文路径
    var basePath = rootPath + '/tms/bas/contractitem_s/';
    ...
    var gridOption = {
        columns: [
            ...
            {
                //行内按钮
                display: '报价', align: '', minWidth: 50, width: '5%', sortable: false,
                render: function (item, index) {
                    return '<a name="edit_fee" class="row-edit-btn" data-index="' + index + '"><i class="iconfont l-icon-price"></i></a>';
                }
            },
            {display: '业务类型', name: 'service_type_name', align: 'left', minWidth: 50, width: '10%', quickSort: false, isSort: false,},
            {display: '费用名称', name: 'fee_name_show', align: 'left', minWidth: 50, width: '10%',},
            {
                display: '是否含税', name: 'is_tax', align: 'left', minWidth: 50, width: '5%',
            ...
            {display: '税点', name: 'tax_point', align: 'left', minWidth: 50, width: '5%'},
            {display: '价格模式', name: 'price_model_name', align: 'left', minWidth: 50, width: '10%', quickSort: false, isSort: false},
            {display: '计价基准', name: 'price_base_name', align: 'left', minWidth: 50, width: '10%', quickSort: false, isSort: false,
                render: LG.render.ref(data_price_base, 'price_base')},
            {display: '计量单位', name: 'unit_name', align: 'left', minWidth: 50, width: '5%', quickSort: false, isSort: false,
                render: renderFeeUnit},
            {display: '对应事件', name: 'event_name', align: 'left', minWidth: 50, width: '10%'},
            {display: '报价记录', name: 'count', align: 'left', minWidth: 50, width: '5%'},
            {display: '合并规则', name: 'combine_type_name', align: 'left', minWidth: 50, width: '5%'},
        ...
        fields: [
            {
                ...
                options: {
                    url: basePath + 'getServiceType?id=' + client_id
                }
            },
            {
                display: "费用名称",
                ...
                validate: {required: true},
                options: {
                    url: rootPath + '/tms/bas/dict/getData?query=fee_type'
                }
            },
            {
                display: "是否含税",
            ...
            },
            {
                ...
                type: "select",
                cssClass: "field",
                options: {url: basePath + 'getPriceModel'},
                validate: {required: true}
            },
            ...
            {display: "计量单位", name: "unit", newline: false, labelWidth: 80, width: 170, space: 30, type: "select", cssClass: "field", validate: {required: true}, comboboxName: 'unit_c',
                options: {data: []}
            },
            ...
            {display: "关联属性", name: "fee_prop", newline: true, labelWidth: 80, width: 450, space: 30, type: "select", cssClass: "field", validate: {required: true},
                options: {
                    url: rootPath + '/tms/bas/dict/getData?query=fee_prop',
        ...
        if (isFirst) {
            liger.get("price_base_c").bind('selected', function (value, text) {
                for (var i = 0; i < data_price_base.length; i++) {
                    if (data_price_base[i].id == value) {
                        var unit_c = liger.get("unit_c");
                        unit_c._setData(data_price_base[i].unitList);
                    }
                }
            });
        }
...
<script type="text/javascript">
    ...
    $(function () {
        ...
        $mainGrid.on('click', 'a[name=edit_fee]', function (e) {
            var index = $(this).data("index");
            var selected = mainGrid.getRow(index);
            var url = rootPath + "/tms/bas/contractfee_s/loadPage?pk_id=" + selected.pk_id + "&contract_id=" + client_id;
